...
def gen_chapter(request, object_id):
    ...
    prompt = 'Сделай хороший аналитический отчет на тему ' + report_description + ' используй следующий контекст \n' + context_docs
    # prompt = 'расскажи какая Ариночка молодец'

    answer = create_completion_openai(client, prompt)
    report.chapter_text_1 = answer
    report.chapter_text_2 = prompt
    report.save()
